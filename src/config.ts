// src/config.ts
export const site = {
  title: 'Agung Tirtayasa Kusuma',
  author: 'Portofolio',
  role: 'Network Engineer',
  tagline: 'Learning by building. Improving one network at a time.',
  cv: '/cv.pdf',
  description: 'Cisco labs, networking projects, and technical documentation focused on routing, switching, network services, and network security',
};

export const navLinks = [
  { label: 'Home',     href: '/' },
  { label: 'Projects', href: '/projects' },
  { label: 'Journal',  href: '/blog' },
  { label: 'About',    href: '/about' },
  { label: 'Contact',  href: '/contact' },
];

export const socials = [
  { label: 'GitHub',   href: 'https://github.com/agungtirtayasakusuma' },
  { label: 'LinkedIn', href: 'https://www.linkedin.com/in/agungtirtayasakusuma' },
  { label: 'Instagram', href: 'https://www.instagram.com/visualoftirtayasa' },
  { label: 'X', href: 'https://x.com/0xargovesta' },
];

export const contact = {
  email: 'agungtirtayasakusumaa@gmail.com',
  location: 'Bekasi, West Java, Indonesia',
  // phone: '+62 812 0000 0000',   // optional — uncomment to display it
  web3formsKey: '70f98aa4-7396-4eb6-8712-7308d20a0970',   // ← free key from https://web3forms.com
};